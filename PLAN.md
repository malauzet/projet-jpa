...
Le projet (étapes 1 à 12, exigences du cahier des charges) et les approfondissements personnels hors périmètre (étapes 13 à 23 : tests, CI, recherche par nom, affichage uniforme, Docker, vitesse d'import + gestion d'erreur, import idempotent, `updateAll`, config DB externalisée, revue complète, présentation du dépôt) sont ✅ terminés à 100 %, `CODE_EXPLANATION.md`/`PLAN.md` sont commit, le tag `v1.0.0` est publié. Reste, en approfondissement personnel supplémentaire uniquement (pas un engagement) :

1. **Nettoyer la valeur `"None"` dans `langue`** — trouvé par l'utilisateur via phpMyAdmin (2026-07-31) : une ligne de la table `langue` a pour `nom` la chaîne littérale `"None"`, clairement pas une vraie langue. Hypothèse la plus probable : `films.json` contient `"langue": "None"` sur au moins un film, plutôt qu'une clé absente ou une valeur `null` JSON — `toLangue`/`dedupe` ne filtrent aujourd'hui que les valeurs vides/absentes (cf. étape 11, bugs #2-3), pas cette valeur "poison" qui ressemble à s'y méprendre à une vraie donnée. Pourrait venir d'un `None` Python (scraping) transformé en texte au lieu de rester `null` au moment de la sérialisation JSON. **À faire plus tard** : vérifier par `grep` sur `films.json` si `"None"` apparaît ailleurs (`pays`, `genre`, `lieuTournage`, `identite`...) avant de corriger, pour traiter tous les cas d'un coup plutôt qu'un par un.
...
**Qualité des données (trouvé le 2026-07-31, pas encore traité) :**
- **Valeur `"None"` dans `langue`** — repéré par l'utilisateur via phpMyAdmin, une ligne de `langue` a pour `nom` la chaîne littérale `"None"`. Hypothèse : `films.json` contient `"langue": "None"` sur au moins un film (peut-être un `None` Python mal sérialisé côté scraping), une valeur "poison" qui passe à travers `toLangue` sans être filtrée (contrairement aux clés absentes/vides déjà gardées, étape 11 bugs #2-3). À vérifier par `grep` sur `films.json` si d'autres champs (`pays`, `genre`, `lieuTournage`, `identite`...) ont le même problème avant de corriger, pour traiter tous les cas ensemble.
