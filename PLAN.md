...
| Audit qualité des données + 2 correctifs de parsing — *hors périmètre, bonus* (étape 24) | 100 % (audit sans perte confirmée, `"None"`/`trim()` corrigés et revalidés) — doublons de `lieu_naissance` documentés mais pas traités, voir section g) |
...
1. **Décider quoi faire des doublons de granularité dans `lieu_naissance`** (étape 24, section g pour le détail complet) — ex. `"Bronx, New York"` / `"Bronx, New York City, New York, USA"` / `"Bronx, New York, USA"` pour le même lieu réel. Contrairement à `"None"`, pas de fix évident : un dédoublonnage automatique par simple regroupement (même ville en tête) fusionnerait à tort des lieux réels différents partageant un nom (`Moscow` Russie/Idaho/Arkansas, `Paris` France/Texas/Kentucky...). Options envisagées, aucune tranchée : ne rien faire (documenter comme limite des données libres) ; fusion manuelle ciblée sur les ~184 groupes identifiés comme "homogènes" ; ou un vrai référentiel géographique (jugé hors de portée).
...
Backlog initial (section g) entièrement épuisé (recherche par nom, résumé de résultats, gestion d'erreurs, import idempotent, `updateAll`, config DB, nettoyage de `"None"` en langue) — le projet est fonctionnellement stable, les prochaines étapes concernent uniquement la présentation du dépôt, la documentation, et cette question ouverte sur `lieu_naissance`.
