...
- **Anomalies détectées dans les données** :
  ...
  - `lieuNaissance` est un champ texte libre unique, contrairement à `lieuTournage` qui décompose au moins ville/étatDept/pays -> aucune structure imposant un niveau de détail fixe, donc un même lieu réel écrit à des granularités différentes selon l'entrée (ex. "Bronx, New York" / "Bronx, New York City, New York, USA" / "Bronx, New York, USA"). Défaut de conception de la source elle-même (un champ général rempli au gré de qui le renseigne, plutôt que des champs distincts et contraints) -> `cleDedoublonnage` ne peut corriger qu'une différence d'écriture d'une valeur par ailleurs identique (casse/accents/espaces), pas un manque de structure. Décision (2026-08-02) : ne pas tenter de fusion automatique (risque de confondre à tort deux lieux réels différents partageant un nom, ex. plusieurs "Moscow"/"Paris" bien distincts existent dans le dataset) -> laissé tel quel.
